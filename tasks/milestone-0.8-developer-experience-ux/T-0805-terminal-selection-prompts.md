...
- `cli/prompt.ts` — Terminal interaction and layout utilities:
  - `selectPrompt`: Interactive terminal prompt supporting arrow-key selection and numbered choice fallback.
  - `formatTable`: Formatted, column-aligned ASCII/Unicode table rendering for status tables.
- `tests/unit/cli_prompt_test.ts` — Unit tests verifying choice selection parsing, arrow-key escape sequences, and table padding alignment.
...
1. Given `selectPrompt` is called in an interactive terminal, when rendered, then choices are displayed with an indicator pointing to the active selection.
2. Given a non-interactive stdin or fallback input stream, when a choice index or label is received, then the corresponding choice value is resolved.
3. Given `formatTable` is called with headers and rows, then it returns a formatted string with consistent column widths, borders, and cell padding.

## Tests required

- [ ] Unit — `tests/unit/cli_prompt_test.ts`: Verify `selectPrompt` selection resolution, default index handling, and `formatTable` border/column alignment.
