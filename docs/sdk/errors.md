...
Every platform error in RailFog maps to one of the 10 exhaustive, machine-readable error codes defined in `PLAT-12`. Clients check the error code rather than parsing unstable human-readable error messages.
...
const handler: FunctionHandler = async (req, ctx) => {
  try {
    ...
    if (!result.ok) {
      throw new ConflictError("Version conflict on counter update", ctx.requestId);
    }

    return Response.json({ success: true });
  } catch (err) {
    const normalized = normalizeError(err, ctx.requestId);
    console.error(`Request failed with code [${normalized.code}]: ${normalized.message}`);
