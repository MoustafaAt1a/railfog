...
Use these words, and only these words, for these concepts. Two models generating
code in the same repo will drift in naming unless both are pinned to the same
list. If a concept below has a spelling variant elsewhere in either uploaded
spec file, this list wins (the LTS file's usage matches this list already).
...
new concept appears, add it here in the same task that introduces it — don't let
it live undocumented.
