...
> [PLAT-11 (Route Specificity)](../contracts/platform.contract.md#PLAT-11)
> &nbsp;|&nbsp; **Algorithm**: Deterministic Numeric Score

When an incoming HTTP request arrives at the Gateway or Runtime, multiple
configured route patterns may match the URL path. RailFog resolves route
ambiguity deterministically using the **Route Specificity Scoring Formula**
(`PLAT-11`).
...
- **literal_segments**: Number of exact, static path segments (each contributes
  2 points).
...
- **Tie-Breaker Rule**: If scores are identical, declaration order in
  `railfog.toml` breaks ties (the first declared route wins).
