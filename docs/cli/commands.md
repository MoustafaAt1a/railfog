...
This document is the complete alphabetical reference for all commands supported by the `rail` CLI.
...
rail check [path]
```
Validates `railfog.toml`, verifies that all entrypoints exist without parent traversal (`..`), computes `PLAT-11` route specificity scores, and flags SSRF violations (`PLAT-5`).
...
```
Boots local development runtime with SQLite KV and queue storage and local filesystem object storage (`PLAT-17`).
...
```
Bundles TypeScript source artifacts, computes SHA-256 hashes (`OBJ-4`), creates an immutable revision (`FN-3`), pre-warms sandboxes, and executes an atomic traffic flip.
...
```
Executes an instant atomic pointer-flip rollback to a previous revision without rebuilding.
...
```
Manages encrypted project secrets. Secret values are never displayed in `list` and are auto-redacted in runtime logs.
...
```
Evaluates URL path resolution against `railfog.toml` offline and prints the winning route's specificity score.
